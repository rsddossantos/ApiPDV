...
<div class="loginArea">
    ...
    <form method="POST">
        @csrf
        <input class="form-control" type="text" name="name" id="name" placeholder="Digite seu nome" spellcheck="false" required />
        <input class="form-control" type="email" name="email" id="email" placeholder="Digite seu e-mail" spellcheck="false" required />
        ...
        Já tem cadastro? <a href="{{url('/api/web/login')}}">Faça seu Login</a>
    </form>
</div>
        ...
        $.ajax({
            ...
            success:function(json){
                if(json.error) {
                    ...
                    window.location.href = '/api/web/user?token='+json.token+'&user='+json.data.name;
                }
            },
            error:function(){
